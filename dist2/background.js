...
let disconnectList = {}; //create list to store the retrived list
...
function fetchDisconnectList() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const response = yield fetch(DISCONNECT_URL);
            disconnectList = yield response.json();
...
chrome.webRequest.onBeforeRequest.addListener(function (details) {
    if (!details.url)
        return; // Check if URL is defined
...
function storeDetectedTracker(trackerInfo) {
    chrome.storage.local.get(['selectedProfile', 'detectedTrackers', 'uniqueTrackersCount', 'uniqueDomainsCount'], (data) => {
        ...
        const isDuplicateTracker = trackers[profile].some((entry) => entry.trackerDomain === trackerInfo.trackerDomain);
        const isDuplicateParentDomain = trackers[profile].some((entry) => entry.parentDomain === trackerInfo.parentDomain);
